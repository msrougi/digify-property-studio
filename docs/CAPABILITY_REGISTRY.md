...
| `reflection.analyze` | Vision | Mede quanto conteúdo de baixo gradiente (reflexo/brilho difuso) uma cena tem, rodando de verdade o algoritmo de supressão de reflexo (Yang et al., CVPR 2019 — geometria/matemática clássica, EDP resolvida via DCT, não é IA treinada) numa resolução reduzida. Não localiza/segmenta a superfície reflexiva (sem sinal real disponível pra isso — duas rodadas de pesquisa de modelos, ver `docs/vision/REFLECTION.md`). | shipped (correção de cena inteira, não localizada) |
...
| `reflection.act` | Production | Roda o mesmo algoritmo real numa resolução maior e gera um overlay de imagem cobrindo o frame inteiro da cena, composto via Rendering Engine (`docs/vision/REFLECTION.md`). Confidence sempre `confirm` — nunca auto-executa. | shipped |
