...
| `perspective.analyze` | Vision | Detecta inclinação real do horizonte E de linhas verticais (quinas de parede, batentes de porta) via Sobel + Transformada de Hough (geometria clássica, não IA), verificado contra vídeos sintéticos com ângulo conhecido. Distorção de lente grande angular (barril/pincushion) ainda não implementada. Ver `docs/vision/PERSPECTIVE.md`. | shipped (horizonte + verticais) |
| `reflection.analyze` | Vision | Detecta superfícies reflexivas e objetos indesejados refletidos (equipe/tripé). Pesquisa exaustiva feita (MirrorNet, GDNet, 3DRef, ADE20K/CSAILVision, ONNX Model Zoo, Objects365) — todo modelo real com a classe certa hospeda pesos fora do allowlist de rede deste ambiente. Ver `docs/vision/REFLECTION.md`. | planned (sem sinal real disponível) |
...
| `perspective.act` | Production | Nivela o horizonte (rotação + recorte pela maior área sem cantos pretos + reescala) e/ou endireita linhas verticais (cisalhamento via filtro `perspective` do FFmpeg + recorte seguro + reescala), independentemente e combinados na mesma cadeia (`docs/vision/PERSPECTIVE.md`). Cada correção clampada em ±8°, confidence 75 ("confirm") — nunca auto-aplica. | shipped (horizonte + verticais) |
